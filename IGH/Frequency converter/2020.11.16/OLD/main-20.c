...
void DriverEtherCAT()
{
    ...
    default:
    {
        ...
        if (!(cycle_counter % 1000))
        {
            ...
            EC_WRITE_U16(domainOutput_pd + ECT_Frequency, 0x1388);
            printf("                                                                                 ");
            printf("ECT_Status:0x%x\t", EC_READ_U16(domainInput_pd + ECT_Status));
            printf("ECT_Output_frequency:0x%x\t", EC_READ_U16(domainInput_pd + ECT_Output_frequency));
            printf("ECT_Command:0x%x\n", EC_READ_U16(domainInput_pd + ECT_Command));
            // servo 0
