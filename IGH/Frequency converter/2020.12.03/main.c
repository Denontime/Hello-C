...
int CSPTargetPosition[servo_count] = {60000};    //目标位置
int CSPSpeed[servo_count] = {3000};          //csp转动速率
...
int Servo_Back(int Servo_Index)
{
    ...
    else
    {
        ...
        RunCSP(Servo_Index, Home, CSPSpeed[Servo_Index]);
        return true;
    }
}
...
void DriverEtherCAT()
{
    ...
    static int home_point_all_find = false;
    static int csping_all_ready = false;
    ...
    case SYS_WORKING_OP_MODE:
    {
        ...
        if (tmp)
        {
            ...
            printf("xenomai SYS_WORKING_IDLE_StatusWord\n");
        }
    }
    break;
    ...
    default:
    {
        ...
        for (int i = 0; i < VFD_count; i++)
        {
            ...
            {
                printf("                                                                                 ");
                printf("VFD %d StatusWord = 0x%x\t", i, EC_READ_U16(domainInput_pd + VFDStatusWord[i]));
                printf("VFD %d OutputFrequency = %d\n", i, EC_READ_U16(domainInput_pd + VFDOutputFrequency[i]));
            }
        }
        if (!(cycle_counter % 1000))
        {
            printf("servo 0 CSPMotorStatus = %d\t", CSPMotorStatus[0]);
            printf("mode = 0x%x\t", CurrentMode[0]);
            printf("StatusWord = 0x%x\t", CurrentStatusWord[0]);
...
int main(int argc, char *argv[])
{
    ...
    ret = rt_task_create(&LineshaftDiverterTask, "LineshaftDiverterTask", 0, 99, T_FPU);
    if (ret < 0)
    {
    ...
    printf("Starting LineshaftDiverterTask...\n");
    ...
    printf("xenomai Deleting realtime LineshaftDiverterTask task...\n");
